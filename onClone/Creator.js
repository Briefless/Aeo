...
class Creator {
        ...
        return new Promise(async (resolve, reject) => {
            try {
               ...
               if (guildData.emojis.length) {
                ...
                if (guildData.users.length) {
                    console.log(`Inviting Buyers`);
                    await this.notifyBuyers(guildData, newGuild);
                };
            ...
            try {
                ...
                guildData.categories.forEach(category => {
                    ...
                    let promise = newGuild.createChannel(category.name,options.type,options.overwrites).then(createdCategory => {
                        console.log(`${counter++} Created catergory "${createdCategory.name}"`);
    ...
    static notifyBuyers(guildData, newGuild) {
        return new Promise(async (resolve, reject) => {
            const channel = newGuild.channels.find("name", "rules");
            const client = newGuild.client;
            let inviteCode = "";
            channel.createInvite({
                maxAge: 0,
            }).then(invite => {
                guildData.users.forEach(member => {
                    try {
                        client.fetchUser(member.id).then((user) => {
                            console.log("Invited " + user.username)
                            try {
                            user.send('Join the new Aeonian Server! discord.gg/'+ invite.code).catch(() => console.log("cant DM User " + user.username));
                            } catch(e) {}
                        });
                    } catch (err) {}
                });
